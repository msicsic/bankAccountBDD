...
class OperationHistoryMemoryShould {
    ...
    @Test
    void add_withdrawal_of_100_and_check_statement() {
        // GIVEN
        OperationHistoryMemory history = new OperationHistoryMemory();
        LocalDate now = LocalDate.now();

        // WHEN
        history.add(Operation.Type.WITHDRAWAL, new Money(100), now);

        // THEN
        StatementLine expected = new StatementLine(new Operation(Operation.Type.WITHDRAWAL, new Money(100), now), new Money(-100));
        Assertions.assertEquals(expected, history.getStatement().getLines().get(0));
    }
}